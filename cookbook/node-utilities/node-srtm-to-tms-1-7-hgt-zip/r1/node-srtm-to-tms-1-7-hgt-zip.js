...
if ( row > rowEnd - 20 && row < rowEnd ) console.log(  'row', row );
...
console.log( 'cropFile.length', cropFile.length );
		...
		for ( var i = 0; i < cropFile.length; ) {
			...
			elevation = ( cropFile[ i++ ] >> 256 ) + cropFile[ i++ ];
...
console.log( '\nelevations.length', elevations.length );
...
console.log( typedArray.length );
		...
		zip.file( zoomText + '.' + TMS7plusX + '.' + TMS7plusY + '.hgt', typedArray );
		...
		if ( runType === 'test' ) {
			...
			fName = outputDir + TMS7plusX + '/' + TMS7plusY + '.zip';
