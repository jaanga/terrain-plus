	...
	var appTitle = 'HGT Viewer R5';
	var folderSource = '../../../data-samples/';
	var fileList = 'data-samples-files-list.txt';
...
//	var fileWidth, fileHeight;
	...
	function parseData( arrayBuffer, fname) {
		...
		elevations = [];
		var index = 0;
		for (var i = 0, len = arrayBuffer.length; i < len; i++) {
			elevations.push( arrayBuffer[ index++ ] * 256 + arrayBuffer[ index++ ] );
		}
console.log( 'start', fname );
		...
		var i, index = 0, len = imageDataData.length;
		if ( inpPretty.checked === true ) {
			for ( i = 0; i < len; i++ ) {
				elevation = arrayBuffer[ index++ ] * 256 + arrayBuffer[ index++ ];
